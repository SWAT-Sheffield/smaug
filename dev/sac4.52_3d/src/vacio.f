...
namelist /filelist/   filenameini,filename,varnames, typefileini,typefileout,&
   typefilelog,snapshotini,snapshotout,fullgridini,fullgridout,dixBmin1,&
   dixBmin2,dixBmin3,dixBmax1,dixBmax2,dixBmax3
namelist /savelist/   tsave,itsave,dtsave,ditsave
namelist /stoplist/   itmax,tmax,tmaxexact,dtmin,residmin,residmax,t,it,&
   cputimemax
namelist /methodlist/ wnames,fileheadout,eqpar,typeadvance,typefull,typepred,&
   typeimpl,typefilter,typelimiter,typeentropy,entropycoef,artcomp,&
   typelimited,typefct,typetvd,typeaxial,typepoisson,typeconstrain,&
   constraincoef,useprimitive,muscleta,musclomega,acmwidth,acmnolim,acmcoef,&
   acmexpo,fourthorder,implmrpc,dimsplit,typedimsplit,sourcesplit,&
   typesourcesplit,sourceunsplit,divbfix,divbwave,divbconstrain,angmomfix,&
   compactres,smallfix,smallp,smallpcoeff,smallrho,smallrhocoeff,vacuumrho,&
   nproc,procpar
namelist /boundlist/  nB,typeB,ixBmin,ixBmax,idimB,upperB,extraB,typeBscalar,&
   ipairB
namelist /paramlist/  courantpar,dtpar,dtdiffpar,dtcantgrow,slowsteps,&
   ...
   implrestart,implrestart2,impliter,impliternr,typeimplinit,typeimpliter,&
   typeimplmat,implnewton,implconserv,implcentered,implnewmat,implpred,&
   impl3level,impljacfast,implsource
!-----------------------------------------------------------------------------
...
dixBmin1=2;dixBmin2=2;dixBmin3=2;dixBmax1=2;dixBmax2=2;dixBmax3=2; !Default width of boundary regions
...
if(oktest) then 
   ...
   enddo
   write(unitterm,*)'Type of ini/out and log files:',typefileini,typefileout,&
      typefilelog
...
do ifile=1,nfile
   ...
   if(tsave(1,ifile)==bigdouble.and.itsave(1,ifile)==biginteger&
      ...
      .and.verbose) write(uniterr,*)'Warning in ReadParameters: ',&
...
if(typefull1=='source'.and.dimsplit.and.sourceunsplit) write(*,&
...
if(ixBmax(1,1)==0.and.oktest)then
   do iB=1,nB
      write(unitterm,'(" TYPEB(",i2,")       = ",100a10)') iB,(typeB(iw,iB),&
         iw=1,nw)
   end do
   write(unitterm,*)' EXTRAB = ',extraB
end if
if(ixBmax(1,1)/=0.and.oktest) write(unitterm,boundlist)
if(dixBmin1>dixBlo.or.dixBmin2>dixBlo.or.dixBmin3>dixBlo.or.dixBmax1&
   >dixBhi.or.dixBmax2>dixBhi.or.dixBmax3>dixBhi) call die( &
...
if(typeimpl1/='nul')then
   ...
   if(implmrpc.and.typeimpliter/='vac_mrpc')call die&
   ...
   if(impljacfast.and.typeimpl1/='tvdlf1'.and.typeimpl1/='cd')call &
      die('Error: impljacfast=T works with typeimpl=tvdlf1 or cd!')

   if(typeimpliter=='vac_mrpc')then
      if(implnewton)call die('Error: MRPC with Newton-Raphson iteration!')
      if(residmin>zero.and.typeimplinit/='nul')write(*,*)&
         'Warning: MRPC for steady state',' should not use typeimplinit=',&
         typeimplinit
   endif
...
if(typefull1=='tvd'.and.typeadvance/='onestep')call die&
   ('tvd method should only be used as a onestep method')

if(typefull1=='tvd' .and. .not.dimsplit)write(*,*)&
...
if(typefileini=='auto')then
   ...
   if(ichar(fhead(1:1))/=0.and.ichar(fhead(2:2))/=0.and. ichar(fhead(3:3))&
      /=0.and.ichar(fhead(4:4))/=0)then
      typefileini='ascii'
   else if(ichar(fhead(89:89))==0.and.ichar(fhead(90:90))==0&
      .and. (ichar(fhead(88:88))==24.or.ichar(fhead(91:91))==24))then
      typefileini='binary'
   else
      typefileini='special'
   endif
   if(verbose)then
      ...
      if(typefileini=='special'.and. ichar(fhead(89:89))==0&
         .and.ichar(fhead(90:90))==0.and. (ichar(fhead(88:88))==20&
         .or.ichar(fhead(91:91))==20)) write(*,*)'Looks like a real*4 file'
   endif
endif
...
do
    ...
    read(unitini,*,iostat=ios)it,t,ndimini,neqparini,nwini
        if(oktest) write(unitterm, &
           "('it=',i7,' t=',g10.3,' ndim=',i3,' neqpar=',i3,' nw=',i3)")it,t,&
           ndimini,neqparini,nwini
        gencoord= ndimini<0
        call checkNdimNeqparNw(ndimini,neqparini,nwini,neqparin,nwin)
    read(unitini,*,iostat=ios)nx
        if(oktest) write(unitterm,"('nx =',3i4)")nx
        call setixGixMix(ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3)
    read(unitini,*,iostat=ios)(eqpar(ieqpar),ieqpar=1,neqparin),(eqparextra,&
       ieqpar=neqparin+1,neqparini)
    ...
    read(unitini,*,iostat=ios)(x(ix1,ix2,ix3,idim),idim=1,ndim),(w(ix1,ix2,&
       ix3,iw),iw=1,nwin),(wextra,iw=nwin+1,nwini)
    enddo;enddo;enddo;
    if(ios/=0)then
...
if(oktest) write(*,*)'x,w:',x(ixtest1,ixtest2,ixtest3,idimtest),w(ixtest1,&
   ixtest2,ixtest3,iwtest)
if(oktest) write(*,*)'x,w:',x(ixtest1,ixtest2,ixtest3,idimtest),w(ixtest1,&
   ixtest2,ixtest3,1:nw)
...
integer:: ios                           ! 0 if not EOF, -1 if EOF, >0 if error
integer:: ndimini,neqparini,neqparin,nwini,nwin ! values describing input data
integer:: ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3,idim,iw,ieqpar,snapshot
...
if(neqpar+nspecialpar/=neqparini)write(*,"(a,i3,a,i3)")&
   'Warning in ReadFileini: number of eq.params=',neqpar,' /= neqparini=',&
   neqparini

if(nw/=nwini)write(*,"(a,i3,a,i3)")&
   'Warning in ReadFileini: number of variables nw=',nw,' /= nwini=',nwini

if((neqpar+nspecialpar/=neqparini.or.nw/=nwini).and.varnames&
   =='default')call die&
...
!=============================================================================
subroutine setixGixMix(ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3)

include 'vacdef.f'
integer:: ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3
!-----------------------------------------------------------------------------

ixGmin1=ixGlo1;ixGmin2=ixGlo2;ixGmin3=ixGlo3;
ixMmin1=ixGmin1+dixBmin1;ixMmin2=ixGmin2+dixBmin2;ixMmin3=ixGmin3+dixBmin3;
...
ixMmax1=ixMmin1+nx(1)-1;ixMmax2=ixMmin2+nx(2)-1;ixMmax3=ixMmin3+nx(3)-1;
ixGmax1=ixMmax1+dixBmax1;ixGmax2=ixMmax2+dixBmax2;ixGmax3=ixMmax3+dixBmax3;

! Set the index range for this grid
if(fullgridini)then
   ixmin1=ixGmin1;ixmin2=ixGmin2;ixmin3=ixGmin3;ixmax1=ixGmax1;ixmax2=ixGmax2
   ixmax3=ixGmax3;
   
else
   ixmin1=ixMmin1;ixmin2=ixMmin2;ixmin3=ixMmin3;ixmax1=ixMmax1;ixmax2=ixMmax2
   ixmax3=ixMmax3;
endif

if(ixGmax1>ixGhi1.or.ixGmax2>ixGhi2.or.ixGmax3>ixGhi3)then
   write(uniterr,*)'Stop: nxhi=',ixGhi1-dixBmax1-ixMmin1+1,ixGhi2-dixBmax2&
      -ixMmin2+1,ixGhi3-dixBmax3-ixMmin3+1
   call die('Error in SetixGixMix')
end if

nx1=nx(1);nx2=nx(2);nx3=nx(3);
...
write(physics,'(a,i1,i1)')typephys,3,3

i=index(fileheadout,'_')
if(i>=1)then
   if(physics/=fileheadout(i+1:i+10))then
      write(*,*)'This code is configured to ',physics
      call die('Error:  physics in file is '//fileheadout(i+1:i+10))
   endif
else
   i=79-len(typephys)-3
   do
...
include 'vacdef.f'
integer:: ifile,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3
double precision:: w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,nw)
...
select case(ifile)
      ...
      if(snapshotout>0.and.isaveout>0)then
         if(isaveout==snapshotout*(isaveout/snapshotout))then
            ...
            filenameout=filenameout(1:index(filenameout,' ')&
               -1)//'_'// itstring(10-int(alog10(isaveout+1.5)):10) 
         endif
      endif
      isaveout=isaveout+1
      if(fullgridout)then
         ixmin1=ixGmin1;ixmin2=ixGmin2;ixmin3=ixGmin3;ixmax1=ixGmax1
         ixmax2=ixGmax2;ixmax3=ixGmax3;
         
      end if
      select case(typefileout)
         case('ascii')
            call savefileout_asc(unitini+ifile,w,ixmin1,ixmin2,ixmin3,ixmax1,&
               ixmax2,ixmax3)
         case('binary')
            call savefileout_bin(unitini+ifile,w,ixmin1,ixmin2,ixmin3,ixmax1,&
               ixmax2,ixmax3)
         case default
   ...
   case(filelog_)
      select case(typefilelog)
         case('default')
            call savefilelog_default(unitini+ifile,w,ixmin1,ixmin2,ixmin3,&
               ixmax1,ixmax2,ixmax3)
         case default
...
!=============================================================================
subroutine savefileout_asc(qunit,w,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3)
...
integer:: qunit,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3,ix1,ix2,ix3,iw,idim,&
   ndimout
double precision:: w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,nw),qw(nw)
logical:: fileopen
!-----------------------------------------------------------------------------
...
write(qunit,"(3i4)") ixmax1-ixmin1+1,ixmax2-ixmin2+1,ixmax3-ixmin3+1
...
do ix1= ixmin1,ixmax1 
   ...
   where(abs(w(ix1,ix2,ix3,1:nw))>5.0d-16)
      qw(1:nw)=w(ix1,ix2,ix3,1:nw)
   elsewhere
      qw(1:nw)=0d0
   end where
   write(qunit,"(100(1pe18.10))")x(ix1,ix2,ix3,1:ndim),qw(1:nw)
enddo;enddo;enddo;
...
!=============================================================================
subroutine savefileout_bin(qunit,w,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3)
...
integer:: qunit,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3,idim,iw,ndimout
double precision:: w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,nw)
logical:: fileopen

!**************** slice
integer:: s_ixmax1,s_ixmax2,s_ixmax3, prom_ixmax1,prom_ixmax2,prom_ixmax3,&
    s_ixmin1,s_ixmin2,s_ixmin3, prom_ixmin1,prom_ixmin2,prom_ixmin3  
!**************** endslice
...
write(qunit) ixmax1-ixmin1+1,ixmax2-ixmin2+1,ixmax3-ixmin3+1
write(qunit)eqpar
write(qunit)varnames
write(qunit)(x(ixmin1:ixmax1,ixmin2:ixmax2,ixmin3:ixmax3,idim),idim=1,ndim)

! write(qunit)w(ix^S,1:nw) produces segmentation fault on Alpha, thus loop

do iw=1,nw
   write(qunit)w(ixmin1:ixmax1,ixmin2:ixmax2,ixmin3:ixmax3,iw)
end do
...
!=============================================================================
subroutine savefilelog_default(qunit,w,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,&
   ixmax3)
...
integer:: qunit,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3
double precision:: w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3,nw)
integer:: iw
logical:: fileopen
...
do iw=1,nw 
   wmean(iw)=sum(dvolume(ixmin1:ixmax1,ixmin2:ixmax2,ixmin3:ixmax3)&
      *w(ixmin1:ixmax1,ixmin2:ixmax2,ixmin3:ixmax3,iw))/volume
