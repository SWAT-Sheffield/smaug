...
INTEGER,PARAMETER:: ixGhi1=1976,ixGhi2=44,ixGhimin=44,ixGhimax=1976
...
PARAMETER (nmpibuffer=dixBhi*maxndimnw*ixGhi1*ixGhi2/ixGhimin)
! MPI header file
include 'mpif.h'
...
INTEGER:: ipe, ipe1,ipe2, npe, npe1,npe2, nxall1,nxall2, nxpe1,nxpe2, ierrmpi
INTEGER:: ixPEmin1,ixPEmin2, ixPEmax1,ixPEmax2
LOGICAL:: mpiupperB,mpilowerB
DOUBLE PRECISION:: sendbuffer
...
INTEGER:: ixtest1,ixtest2,ixtest3,iwtest,idimtest,ipetest     
...
! end include vacdef
!##############################################################################
COMMON /DOUB/ sendbuffer(nmpibuffer),recvbuffer(nmpibuffer,2),&
   tmp(ixGlo1:ixGhi1,ixGlo2:ixGhi2),tmp2(ixGlo1:ixGhi1,ixGlo2:ixGhi2),&
   x(IXGlo1:IXGhi1,IXGlo2:IXGhi2,ndim),dx(IXGlo1:IXGhi1,IXGlo2:IXGhi2,ndim),&
   volume,dvolume(IXGlo1:IXGhi1,IXGlo2:IXGhi2),area(IXGLO1:IXGHI1),&
   areaC(IXGLO1:IXGHI1),areadx(IXGLO1:IXGHI1),areaside(IXGLO1:IXGHI1),&
   surfaceC(2,2,ndim), normalC(2,2,ndim,ndim),fixB1(-dixBlo:dixBhi,&
   ixGLO2:ixGHI2,nw),fixB2(ixGLO1:ixGHI1,-dixBlo:dixBhi,nw),eqpar(neqpar&
   ...
   ixGlo2:ixGhi2,nw),residual,residmin,residmax,t,tmax,dt,dtmin,cputimemax,&
   tsave(nsavehi,nfile),tsavelast(nfile),dtsave(nfile),maxviscoef
COMMON /INTE/ ipe, ipe1,ipe2, npe, npe1,npe2, nxall1,nxall2, nxpe1,nxpe2,&
    ierrmpi,ixPEmin1,ixPEmin2, ixPEmax1,ixPEmax2,unitpar,nerror(nerrcode),&
   kr(3,3),lvc(3,3,3),ixMmin1,ixMmin2,ixMmax1,ixMmax2,ixGmin1,ixGmin2,ixGmax1,&
   ixGmax2,nx1,nx2,nx(ndim),dixBmin1,dixBmin2,dixBmax1,dixBmax2,nB,&
   ixBmin(ndim,nhiB),ixBmax(ndim,nhiB),idimB(nhiB),ipairB(nhiB),slowsteps,&
   nwimpl,nimpl,implrestart,implrestart2,impliter,impliternr,implmrpcpar,&
...
COMMON /LOGI/ mpiupperB(ndim),mpilowerB(ndim),verbose,gencoord, polargrid,&
   upperB(nhiB),fixedB(nw,nhiB),nofluxB(nw,ndim),extraB,dtcantgrow,&
   implconserv,implnewton,implcentered,implnewmat,implpred,impl3level,&
   impljacfast,implsource,dimsplit,sourcesplit,sourceunsplit,artcomp(nw),&
   useprimitive,divbfix,divbwave,divbconstrain,angmomfix,compactres,smallfix,&
   acmnolim, fourthorder,tmaxexact,fullgridini,fullgridout
