    ...
    for(int dim=0; dim<=1; dim++)
     ...
     for(int dim=0; dim<=1; dim++)
     {
       ...
       /*    for(int dim=0; dim<=1; dim++)
	     ...
	     for(int f=0; f<=1; f++)            
	     {
               cuhyperdifvisc(&p,&w,&wnew,&d_p,&d_w,&d_wnew,&d_wmod, &d_dwn1, &d_wd,order,&d_wtemp,b1+f,dim,0);
               cuhyperdifvisc(&p,&w,&wnew,&d_p,&d_w,&d_wnew,&d_wmod, &d_dwn1, &d_wd,order,&d_wtemp,b1+f,dim,1);
                for(ii1=0;ii1<=1;ii1++)
                {
