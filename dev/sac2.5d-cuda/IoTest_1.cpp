...
double ***u=(double ***)calloc(ni,sizeof(double **));
double ***v=(double ***)calloc(ni,sizeof(double **));
double **b=(double **)calloc(ni,sizeof(double *));


// Define h
double ***h=(double ***)calloc(ni,sizeof(double **));

printf("here in runsim\n");
for(i1=0;i1<ni;i1++)
{
    b[i1]=(double *)calloc(nj,sizeof(double));
    u[i1]=(double **)calloc(nj,sizeof(double *));
    v[i1]=(double **)calloc(nj,sizeof(double *));
    h[i1]=(double **)calloc(nj,sizeof(double *));
    for(i2=0;i2<nj;i2++)
    {
	u[i1][i2]=(double *)calloc(2,sizeof(double ));
	v[i1][i2]=(double *)calloc(2,sizeof(double ));
	h[i1][i2]=(double *)calloc(2,sizeof(double ));
    }
}
    ...
    for(i=1;i<(ni-1);i++)
       ...
       // ind=i+j*ni;
        ...
        {   
            ...
            //j=ind/ni;		            
           // i=ind-ni*(int)(ind/ni);		
		u[i][j][1] = (((u[i+1][j][0] + u[i-1][j][0] + u[i][j+1][0] + u[i][j-1][0]))/4) 
- 0.5*(dt/dx)*(((u[i+1][j][0]*u[i+1][j][0]/2) - (u[i-1][j][0]*u[i-1][j][0]/2))) 
- 0.5*(dt/dy)*(v[i][j][0])*(u[i][j+1][0] - u[i][j-1][0]) 
- 0.5*g*(dt/dx)*(h[i+1][j][0]-h[i-1][j][0]);
...
//                - 0.5*(dt/dy)*(v(i,j,1))*(u(i,j+1,1) - u(i,j-1,1)) - 0.5*g*(dt/dx)*(h(i+1,j,1)-h(i-1,j,1));
		...
		//u[i+j*ni+ni*nj] = ((u[i+1+j*ni] + u[i-1+j*ni] + u[i+(j+1)*ni] + u[i+(j-1)*ni])/4)- 0.5*(dt/dx)*(((u[(i+1)+ni*j])*u[(i+1)+ni*j])/2) - ((u[i-1+j*ni]*u[i-1+j*ni])/2) - 0.5*(dt/dy)*(v[i+j*ni])*(u[i+(j+1)*ni] - u[i+(j-1)*ni]) - 0.5*g*(dt/dx)*(h[i+1+j*ni]-h[i-1+j*ni]);
            ...
            v[i][j][1] = ((v[i+1][j][0] + v[i-1][j][0] + v[i][j+1][0] + v[i][j-1][0])/4)
- 0.5*(dt/dy)*(((v[i][j+1][0] * v[i][j+1][0])/2) - ((v[i][j+1][0] * v[i][j+1][0])/2))
- 0.5*(dt/dx)*(u[i][j][0])*(v[i+1][j][0] - v[i-1][j][0]) 
- 0.5*g*(dt/dy)*(h[i][j+1][0]-h[i][j-1][0]);
   ...
   //             - 0.5*(dt/dx)*(u(i,j,1))*(v(i+1,j,1) - v(i-1,j,1)) - 0.5*g*(dt/dy)*(h(i,j+1,1)-h(i,j-1,1));
	...
	//v[i+j*ni+ni*nj] = ((v[i+1+j*ni] + v[i-1+j*ni] + v[i+(j+1)*ni] + v[i+(j-1)*ni])/4)- 0.5*(dt/dy)*((v[i+ni*(j+1)]*v[(i)+ni*(j+1)])/2 - (v[i+(j-1)*ni]*v[i+(j-1)*ni])/2) - 0.5*(dt/dx)*(u[i+j*ni])*(v[i+1+j*ni] - v[i-1+j*ni]) - 0.5*g*(dt/dy)*(h[i+(j+1)*ni]-h[i+(j-1)*ni]);		
              ...
              h[i][j][1] = ((h[i+1][j][0] + h[i-1][j][0] + h[i][j+1][0] + h[i][j-1][0])/4)
- 0.5*(dt/dx)*(u[i][j][0])*( (h[i+1][j][0]-b[i+1][j]) - (h[i-1][j][0]-b[i-1][j]) ) 
- 0.5*(dt/dy)*(v[i][j][0])*( (h[i][j+1][0]-b[i][j+1]) - (h[i][j-1][1]-b[i][j-1]) ) 
- 0.5*(dt/dx)*(h[i][j][0]-b[i][j])*(u[i+1][j][0]- u[i-1][j][0])
- 0.5*(dt/dy)*(h[i][j][0]-b[i][j])*(v[i][j+1][0] - v[i][j-1][0]);
    ...
    if(steeringenabled==1)
    {
      ...
      for(i1=0;i1<nj;i1++)
      {
        for(j1=0;j1<ni;j1++)
	{
          fprintf(fdt,"%f %f %f",u[i1][j1][0],v[i1][j1][0],h[i1][j1][0]);
        }        
        fprintf(fdt,"\n");
      }
      fclose(fdt);
 ...
 //disp('writing data');
    //if finishsteering==1
      ...
      for( i1=0;i1<ni;i1++)
      {
        for( j1=0;j1<nj;j1++)
	{
          fprintf(fd,"%f %f %f",u[i1][j1][0],v[i1][j1][0],h[i1][j1][0]);
	}
        fprintf(fd,"\n");
      }
