...
dwn1[(NVAR*(p->n[0])*(p->n[1]))+fencode_hdmne3(p,i,j,mom1+ii0)]=wtemp[fencode_hdmne3(p,i,j,tmp6)];
 ...
 // if(i<((p->n[0])) && j<((p->n[1])))
     dwn1[(NVAR*(p->n[0])*(p->n[1]))+fencode_hdmne3(p,i,j,energy)]=(grad1_hdmne3(wtemp,p,i,j,tmp7,ii));
