...
__device__ __host__
int encode_hdb (struct params *dp,int ix, int iy) {
...
__device__ __host__
int fencode_hdb (struct params *dp,int ix, int iy, int field) {
...
__device__ __host__
real evalgrad_hdb(real fi, real fim1, real fip2, real fim2,struct params *p,int dir)
{
 //real valgrad_hdb;
...
__device__ __host__
real evalgrad1_hdb(real fi, real fim1, struct params *p,int dir)
{
 //real valgrad_hdb;
...
__device__ __host__
real grad1l_hdb(real *wmod,struct params *p,int i,int j,int field,int dir)
{
  if(dir == 0)
 {
    return(  ( wmod[fencode_hdb(p,i,j,field)]-wmod[fencode_hdb(p,i-1,j,field)]) /((p->dx[0]))    );
 }
 else if(dir == 1)
 {
    return(  ( wmod[fencode_hdb(p,i,j,field)]-wmod[fencode_hdb(p,i,j-1,field)])/((p->dx[1]))    );
  }
 return 0;
...
__device__ __host__
real grad1r_hdb(real *wmod,struct params *p,int i,int j,int field,int dir)
{
  if(dir == 0)
 {
    return(  ( wmod[fencode_hdb(p,i+1,j,field)]-wmod[fencode_hdb(p,i,j,field)]) /((p->dx[0]))    );
 }
 else if(dir == 1)
 {
    return(  ( wmod[fencode_hdb(p,i,j+1,field)]-wmod[fencode_hdb(p,i,j,field)])/((p->dx[1]))    );
  }
 return 0;
...
__device__ __host__
real grad1_hdb(real *wmod,struct params *p,int i,int j,int field,int dir)
{
 //real valgrad_hdb;
 ...
 return(  (wmod[fencode_hdb(p,i+1,j,field)]-wmod[fencode_hdb(p,i-1,j,field)])/(2.0*(p->dx[0]))    );
 }
 else if(dir == 1)
 {

 return(  (wmod[fencode_hdb(p,i,j+1,field)]-wmod[fencode_hdb(p,i,j-1,field)])/(2.0*(p->dx[1]))    );
  }
 return 0;
}

__device__ __host__
real grad2_hdb(real *wmod,struct params *p,int i,int j,int field,int dir)
{
 //real valgrad_hdb;
...
//return((1.0/(2.0*(p->dx[0])))*(wmod[fencode_hdb(p,i+1,j,field)]-wmod[fencode_hdb(p,i-1,j,field)]));
 return(  ( (p->sodifon)?((16*wmod[fencode_hdb(p,i+1,j,field)]+16*wmod[fencode_hdb(p,i-1,j,field)]-wmod[fencode_hdb(p,i-2,j,field)]-wmod[fencode_hdb(p,i+2,j,field)]-30*wmod[fencode_hdb(p,i,j,field)])/6.0):2.0*(wmod[fencode_hdb(p,i+1,j,field)]-2*wmod[fencode_hdb(p,i,j,field)]-wmod[fencode_hdb(p,i-1,j,field)]))/(2.0*(p->dx[0])*(p->dx[0]))    );
 }
 else if(dir == 1)
 {
...
// return((1.0/(2.0*(p->dx[1])))*(wmod[fencode_hdb(p,i,j+1,field)]-wmod[fencode_hdb(p,i,j-1,field)]));
 return(  ( (p->sodifon)?((16*wmod[fencode_hdb(p,i,j+1,field)]+16*wmod[fencode_hdb(p,i,j,field)]-wmod[fencode_hdb(p,i,j-2,field)]-wmod[fencode_hdb(p,i,j+2,field)]-30*wmod[fencode_hdb(p,i,j,field)])/6.0):2.0*(wmod[fencode_hdb(p,i,j+1,field)]-2.0*wmod[fencode_hdb(p,i,j+1,field)]-wmod[fencode_hdb(p,i,j-1,field)]))/(2.0*(p->dx[1])*(p->dx[1]))    );
  }
 return 0;
}

__device__ __host__
real grad_hdb(real *wmod,struct params *p,int i,int j,int field,int dir)
{
 //real valgrad_hdb;
  ...
  if(dir == 0)
 {
    // valgrad=(2.0/(3.0*(p->dx[0])))*(wmod[fencode(p,i,j,field)]-wmod[fencode(p,i-1,j,field)])-(1.0/(12.0*(p->dx[0])))*(wmod[fencode(p,i+2,j,field)]-wmod[fencode(p,i-2,j,field)]);
//return((1.0/(2.0*(p->dx[0])))*(wmod[fencode_hdb(p,i+1,j,field)]-wmod[fencode_hdb(p,i-1,j,field)]));
 return(  ( (p->sodifon)?((NVAR*wmod[fencode_hdb(p,i+1,j,field)]-NVAR*wmod[fencode_hdb(p,i-1,j,field)]+wmod[fencode_hdb(p,i-2,j,field)]-wmod[fencode_hdb(p,i+2,j,field)])/6.0):wmod[fencode_hdb(p,i+1,j,field)]-wmod[fencode_hdb(p,i-1,j,field)])/(2.0*(p->dx[0]))    );
 }
 else if(dir == 1)
 {
    // valgrad=(2.0/(3.0*(p->dx[1])))*(wmod[fencode(p,i,j,field)]-wmod[fencode(p,i,j-1,field)])-(1.0/(12.0*(p->dx[1])))*(wmod[fencode(p,i,j+2,field)]-wmod[fencode(p,i,j-2,field)]);
// return((1.0/(2.0*(p->dx[1])))*(wmod[fencode_hdb(p,i,j+1,field)]-wmod[fencode_hdb(p,i,j-1,field)]));
 return(  ( (p->sodifon)?((NVAR*wmod[fencode_hdb(p,i,j+1,field)]-NVAR*wmod[fencode_hdb(p,i,j-1,field)]+wmod[fencode_hdb(p,i,j-2,field)]-wmod[fencode_hdb(p,i,j+2,field)])/6.0):wmod[fencode_hdb(p,i,j+1,field)]-wmod[fencode_hdb(p,i,j-1,field)])/(2.0*(p->dx[1]))    );
  }
 return 0;
}
