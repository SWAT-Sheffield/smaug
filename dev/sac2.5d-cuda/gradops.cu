...
real grad1l3_MODID(real *wmod,struct params *p,int *ii,int field,int dir)
{
 ...
 if((dir == 0) && i>0 && i<((p->n[0])-1))
 {
   ...
   #ifdef USE_SAC_3D
	   if((i==(p->n[0])-2) || (i==(p->n[0])-3)  && j>1   && j<(p->n[1])-1 && k>1   && k<(p->n[2])-1 )
	       grad=0;
	   else if(i==1 || i==2  && j>0   && j<(p->n[1])-1 && k>0   && k<(p->n[2])-1 )
	       grad=0;
   #else
	   if((i==(p->n[0])-2) || (i==(p->n[0])-3)  && j>1   && j<(p->n[1])-1  )
	       grad=0;
 ...
 else if((dir == 1)    && j>0 && j<((p->n[1])-1))
 {
   ...
   #ifdef USE_SAC_3D
	   if((j==(p->n[1])-2) || (j==(p->n[1])-3)  && i>1   && i<(p->n[0])-1 && k>1   && k<(p->n[2])-1 )
	       grad=0;
	   else if(j==1 || j==2  && i>0   && i<(p->n[0])-1 && k>0   && k<(p->n[2])-1 )
	       grad=0;
   #else
	   if((j==(p->n[1])-2) || (j==(p->n[1])-3)  && i>1   && i<(p->n[0])-1  )
	       grad=0;
 ...
 else if((dir == 2)    && k>0 && k<((p->n[2])-1))
 {
   ...
   if((k==(p->n[2])-2) || (k==(p->n[2])-3)  && i>1   && i<(p->n[0])-1  && j>1   && j<(p->n[1])-1 )
       grad=0;
...
real grad13_MODID(real *wmod,struct params *p,int *ii,int field,int dir)
{
 ...
 if((dir == 0) && i>0 && i<((p->n[0])-1))
 {
    grad=(  ( wmod[encode3_MODID(p,i+1,j,k,field)]-wmod[encode3_MODID(p,i-1,j,k,field)]) /((p->dx[0]))    );
