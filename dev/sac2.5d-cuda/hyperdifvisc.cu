...
__device__ __host__
int encode_hdv (struct params *dp,int ix, int iy) {
...
__device__ __host__
int fencode_hdv (struct params *dp,int ix, int iy, int field) {
...
__device__ __host__
real evalgrad_hdv(real fi, real fim1, real fip2, real fim2,struct params *p,int dir)
{
 //real valgrad_hdv;
...
__device__ __host__
real grad_hdv(real *wmod,struct params *p,int i,int j,int field,int dir)
{
 //real valgrad_hdv;
...
//return((1.0/(2.0*(p->dx[0])))*(wmod[fencode_hdv(p,i+1,j,field)]-wmod[fencode_hdv(p,i-1,j,field)]));
 return(  ( (p->sodifon)?((NVAR*wmod[fencode_hdv(p,i+1,j,field)]-NVAR*wmod[fencode_hdv(p,i-1,j,field)]+wmod[fencode_hdv(p,i-2,j,field)]-wmod[fencode_hdv(p,i+2,j,field)])/6.0):wmod[fencode_hdv(p,i+1,j,field)]-wmod[fencode_hdv(p,i-1,j,field)])/(2.0*(p->dx[0]))    );
 }
 else if(dir == 1)
 {
...
// return((1.0/(2.0*(p->dx[1])))*(wmod[fencode_hdv(p,i,j+1,field)]-wmod[fencode_hdv(p,i,j-1,field)]));
 return(  ( (p->sodifon)?((NVAR*wmod[fencode_hdv(p,i,j+1,field)]-NVAR*wmod[fencode_hdv(p,i,j-1,field)]+wmod[fencode_hdv(p,i,j-2,field)]-wmod[fencode_hdv(p,i,j+2,field)])/6.0):wmod[fencode_hdv(p,i,j+1,field)]-wmod[fencode_hdv(p,i,j-1,field)])/(2.0*(p->dx[1]))    );
  }
 return 0;
}
