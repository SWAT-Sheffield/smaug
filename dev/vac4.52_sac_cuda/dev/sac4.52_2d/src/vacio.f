...
namelist /testlist/   teststr,ixtest1,ixtest2,ixtest3,iwtest,idimtest,&
   ipetest     
namelist /filelist/   filenameini,filename,varnames,      npe1,npe2,&
    typefileini,typefileout,typefilelog,snapshotini,snapshotout,fullgridini,&
...
dixBmin1=2;dixBmin2=2;dixBmax1=2;dixBmax2=2; !Default width of boundary regions
...
! ixtest1,ixtest2 is given for the full grid if ipetest was not set explicitly
...
if(oktest) then 
         write(unitterm,*)'npe^D=',npe1,npe2
...
if(globalixtest)call mpiix(ixtest1,ixtest2,ipetest)
...
if(dixBmin1>dixBlo.or.dixBmin2>dixBlo.or.dixBmax1>dixBhi.or.dixBmax2&
   >dixBhi) call die( 'Error in ReadParameters: adjust dixBlo,dixBhi')
...
do
    ...
    read(unitini,*,iostat=ios)(x(ix1,ix2,idim),idim=1,ndim),(w(ix1,ix2,iw),iw&
       =1,nwin),(wextra,iw=nwin+1,nwini)
    enddo;enddo;
    if(ios/=0)then
...
if(oktest) write(*,*)'x,w:',x(ixtest1,ixtest2,idimtest),w(ixtest1,ixtest2,&
   iwtest)
if(oktest) write(*,*)'x,w:',x(ixtest1,ixtest2,idimtest),w(ixtest1,ixtest2,&
   1:nw)
...
do
    ...
    read(unitini,iostat=ios)(x(ixmin1:ixmax1,ixmin2:ixmax2,idim),idim=1,ndim)
    ! To conform savefileout_bin we use loop for iw
    do iw=1,nwin
       read(unitini,iostat=ios)w(ixmin1:ixmax1,ixmin2:ixmax2,iw)
    end do
    if(ios/=0)then
...
include 'vacdef.f'
integer:: ixmin1,ixmin2,ixmax1,ixmax2,qnx     
!-----------------------------------------------------------------------------

ixGmin1=ixGlo1;ixGmin2=ixGlo2;
ixMmin1=ixGmin1+dixBmin1;ixMmin2=ixGmin2+dixBmin2;

! Shave off ghost cells from nx
if(fullgridini)then
   
   if(ipe1==0)            nx(1)=nx(1)-dixBmin1
   if(ipe1==npe1-1)      nx(1)=nx(1)-dixBmax1
   
   
   if(ipe2==0)            nx(2)=nx(2)-dixBmin2
   if(ipe2==npe2-1)      nx(2)=nx(2)-dixBmax2
   
endif

! Calculate mesh and grid sizes
ixMmax1=ixMmin1+nx(1)-1;ixMmax2=ixMmin2+nx(2)-1;
ixGmax1=ixMmax1+dixBmax1;ixGmax2=ixMmax2+dixBmax2;
...
if(fullgridini)then
   ...
   if(ipe1>0)ixmin1=ixMmin1
        if(ipe2>0)ixmin2=ixMmin2
   if(ipe1<npe1-1)ixmax1=ixMmax1
        if(ipe2<npe2-1)ixmax2=ixMmax2
  
else
   ixmin1=ixMmin1;ixmin2=ixMmin2;ixmax1=ixMmax1;ixmax2=ixMmax2;
endif

if(ixGmax1>ixGhi1.or.ixGmax2>ixGhi2)then
...
call MPI_allreduce(nx1,nxall1,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,ierrmpi)
nxall1=nxall1*npe1/npe;

     
call MPI_allreduce(nx2,nxall2,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,ierrmpi)
nxall2=nxall2*npe2/npe;
...
select case(ifile)
   case(fileout_)
      ...
      select case(typefileout)
         case('ascii')
            call savefileout_asc(unitini+ifile,w,ixmin1,ixmin2,ixmax1,ixmax2)
         case('binary')
            call savefileout_bin(unitini+ifile,w,ixmin1,ixmin2,ixmax1,ixmax2)
         case default
   ...
   case(filelog_)
      select case(typefilelog)
         case('default')
            call savefilelog_default(unitini+ifile,w,ixmin1,ixmin2,ixmax1,&
               ixmax2)
         case default
...
do ix1= ixmin1,ixmax1 
   ...
   where(abs(w(ix1,ix2,1:nw))>5.0d-16)
      qw(1:nw)=w(ix1,ix2,1:nw)
   elsewhere
      qw(1:nw)=0d0
   end where
   write(qunit,"(100(1pe18.10))")x(ix1,ix2,1:ndim),qw(1:nw)
enddo;enddo;
...
write(qunit)(x(ixmin1:ixmax1,ixmin2:ixmax2,idim),idim=1,ndim)

! write(qunit)w(ix^S,1:nw) produces segmentation fault on Alpha, thus loop

do iw=1,nw
   write(qunit)w(ixmin1:ixmax1,ixmin2:ixmax2,iw)
end do
...
do iw=1,nw 
   wmean(iw)=sum(dvolume(ixmin1:ixmax1,ixmin2:ixmax2)*w(ixmin1:ixmax1,&
      ixmin2:ixmax2,iw))/volume
         call mpireduce(wmean(iw),MPI_SUM)
end do
