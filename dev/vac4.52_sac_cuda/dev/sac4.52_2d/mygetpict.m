...
filename=getenv('VACMATFILENAME');
snpict=getenv('VACMATNUMPICT');
npict=sscanf(snpict, '%d')
outname=getenv('VACMATOUTFILENAME');
%for inpict=405:415
%    npict=inpict
%    outname=sprintf('../testascmat_%d_rub1.out',inpict)
...
for i=1:ntot;
    fprintf(ofile,'%f %f %f %f %f %f %f %f %f %f %f %f\n',nascdata(i,:));
end;
fclose(ofile);
%save('outname', 'nascdata',   '-ascii');

%end  %end looping over ipict
exit;
