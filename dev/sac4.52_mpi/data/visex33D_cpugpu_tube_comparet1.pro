...
window, 0,xsize=1025,ysize=400,XPOS = 950, YPOS = 300
;window, 1,xsize=800,ysize=800,XPOS = 500, YPOS = 80
;window, 1,xsize=1025,ysize=1025,XPOS = 950, YPOS = 200 
window, 1,xsize=1025,ysize=300,XPOS = 950, YPOS = 200 
...
;openr,1,'/fastdata/cs1mkg/VAC_NN_tests/3D_tubet1_128_128_128.out',/f77_unf
;openr,1,'/fastdata/cs1mkg/VAC_NN_tests/3D_tubewave1_128_128_128_part1_np010101_000.out',/f77_unf
openr,1,'/fastdata/cs1mkg/VAC_NN_tests/3D_tubewave1_128_128_128_all.out',/f77_unf
...
npic=446
difmean=dblarr(npic/2,13)
difmax=dblarr(npic/2,13)
for pic=0,npic-1 do begin
...
tvframe,rotate(wt(*,*,3)/(wt(*,*,0)+wt(*,*,9)),1),/sample, /bar,title='Vy  sac', $
...
ipic=pic+1
picid=ipic*40
gpudirectory='/fastdata/cs1mkg/smaug/out_tubewave/'
gpuname='3D_atubet1fast_128_128_128_'
;;gpudirectory='/fastdata/cs1mkg/sac_cuda/out_driver_hyp_tube/'
;gpuname='3D_atubet1_128_128_128_'
;;gpuname='3D_atubet1slow_128_128_128_final_'
gpuoutfile=gpudirectory+gpuname+strtrim(string(picid),2)+'.out'




;openr,3,'/data/ap1vf/3D_tube_196_100_100_200s_puls.out',/f77_unf

;openr,3,'/data/ap1vf/3D_tube.ini',/f77_unf
;openr,3,'/data/cs1mkg/sac_cuda/out_withdrv_withbge/3D_tube_128_128_128_5.out'
;openr,3,'/data/cs1mkg/sac_cuda/out_nodrv_withbge/3D_tube_128_128_128_10.out'
;openr,3,'/data/cs1mkg/sac_cuda/out_nodrv_nobge/3D_tube_128_128_128_80.out'
;openr,3,'/data/cs1mkg/sac_cuda/out/3D_tube_128_128_128_10.out'
;openr,3,'/data/cs1mkg/sac_cuda/out_nodrv_nobge_nograv_nohyp/3D_tube_128_128_128_10.out'
;openr,3,'/fastdata/cs1mkg/sac_cuda/out_driver_hyp_tube/3D_atubet1_128_128_128_40.out'
openr,3,gpuoutfile
;;;;while not(eof(1)) do begin
readu,3,headline
...
for iw=0,nw-1 do begin 
 ...
 dwt(*,*,iw)=abs((gwt(*,*,iw)-wt(*,*,iw)))

; difmean(pic,iw)=mean(dwt(*,*,iw)/abs(wt(*,*,iw)))
; difmax(pic,iw)=max(dwt(*,*,iw)/abs(wt(*,*,iw)))

 difmean(pic,iw)=mean(dwt(*,*,iw))
 difmax(pic,iw)=max(dwt(*,*,iw))
...
;	xrange=[xx[xstart]/scale, xx[xend]/scale], $
;	yrange=[zz[zstart]/scale, zz[zend]/scale]
;tvframe,rotate(gwt(*,*,3),1), /bar,title='Vy smaug',$ 
;        /sample, xtitle='x', ytitle='y',charsize=2.0, $
;	xrange=[xx[xstart]/scale, xx[xend]/scale], $
;	yrange=[zz[zstart]/scale, zz[zend]/scale]
...
;	xrange=[xx[xstart]/scale, xx[xend]/scale], $
;	yrange=[zz[zstart]/scale, zz[zend]/scale]
tvframe,rotate(gwt(*,*,3)/(gwt(*,*,0)+gwt(*,*,9)),1),/sample, /bar,title='Vy  smaug', $
        xtitle='x', ytitle='z',charsize=2.0;, CT='dPdT'
...
write_png,'images/vz_slice'+indexss+'.png',image_p, red,green, blue
;write_png,'tubeslice'+indexss+'.png',image_p, red,green, blue
...
plot,difmean(*,0),charsize=2.5,title='rho', psym=5, xtitle='iteration/40', ytitle='absolute difference'
oplot,difmax(*,0), psym=4

plot,difmean(*,4),charsize=2.5,title='e', psym=5, xtitle='iteration/40', ytitle='absolute difference'
oplot,difmax(*,4), psym=4

;plot,difmean(*,7),charsize=1.5,title='bx', psym=5
;oplot,difmax(*,7), psym=4

;plot,difmean(*,6),charsize=1.5,title='by', psym=5
;oplot,difmax(*,6), psym=4

;plot,difmean(*,5),charsize=1.5,title='bz', psym=5
;oplot,difmax(*,5), psym=4

;plot,difmean(*,2),charsize=1.5,title='vx', psym=5
;oplot,difmax(*,2), psym=4

;plot,difmean(*,3),charsize=1.5,title='vy', psym=5
;oplot,difmax(*,3), psym=4

plot,difmean(*,1),charsize=2.5,title='vz', psym=5, xtitle='iteration/40', ytitle='absolute difference'
oplot,difmax(*,1), psym=4

image_p = TVRD_24()

;write_png,vz_slice+indexss+'.png',image_p, red,green, blue
write_png,'images/diftubeactslice'+indexss+'.png',image_p, red,green, blue
close,3
endfor
