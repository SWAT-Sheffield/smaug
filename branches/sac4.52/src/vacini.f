...
do
   ...
   select case(typeini)
      ...
      case('test')
          ...
          call mpiix(ixtest1,ixtest2,ipetest)
      case('typefileini')
          if(verbose)write(*,*)&
      ...
      case('shiftgrid')
               call die('shiftgrid not implemented for MPI')
         call shiftgrid(ixMmin1,ixMmin2,ixMmax1,ixMmax2,w)
      case('sheargrid')
               call die('sheargrid not implemented for MPI')
      ...
      case('setvar')
         ...
         call mpiixlimits(ixmin1,ixmin2,ixmax1,ixmax2)
         do
            if(verbose)write(*,*) &
...
xmin(1) = xmin(1) + dx1*(ixPEmin1-1)
xmax(1) = xmin(1) + dx1*(nx1-1)

     
xmin(2) = xmin(2) + dx2*(ixPEmin2-1)
xmax(2) = xmin(2) + dx2*(nx2-1)
...
call mpiixlimits(ixPmin1,ixPmin2,ixPmax1,ixPmax2)
